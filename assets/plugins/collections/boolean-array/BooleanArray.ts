...
import { and, difference, equals, nand, nor, not, or, xnor, xor } from "./operations";
...
export class BooleanArray {
	...
	constructor(size: number) {
		...
		this.lastChunkMask =
			this.bitsInLastChunk === 0 ? BooleanArray.ALL_BITS_TRUE : ((1 << this.bitsInLastChunk) - 1) >>> 0;
	...
	forEach(
		...
		if (startIndex + count > this.size) {
			throw new RangeError(`Range [${startIndex}, ${startIndex + count}) exceeds array size ${this.size}.`);
		}
	...
	#setRange(startIndex: number, count: number, value: boolean): this {
		...
		if (startChunk === endChunk) {
			...
			const mask =
				count === BooleanArray.BITS_PER_INT && startOffset === 0
		...
		const lastChunkMask =
			lastChunkEndOffset === BooleanArray.CHUNK_MASK
				? BooleanArray.ALL_BITS_TRUE
